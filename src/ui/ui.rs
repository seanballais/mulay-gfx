...
impl UI {
	pub fn new(window: &sdl2::video::Window) -> UI {
		let (egui_painter, egui_state) = egui_sdl2_gl::with_sdl2(window, egui_sdl2_gl::ShaderVersion::Default, egui_sdl2_gl::DpiScaling::Default);
	...
	pub fn draw_frames(&mut self, window: &sdl2::video::Window, app_elapsed_time: f64) {
		...
		egui::Window::new("Console").show(&self.egui_ctx, |ui| {
			ui.add(egui::TextEdit::multiline(&mut self.console_contents.as_str()));

			let textedit_response = ui.add(egui::TextEdit::singleline(&mut self.console_command_contents));
			if textedit_response.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter)) {
				self.console_contents.push_str(&self.console_command_contents.as_str());
		...
		self.egui_painter.paint_jobs(None, textures_delta, paint_jobs);
	}

	pub fn process_input(&mut self, window: &sdl2::video::Window, event: sdl2::event::Event) {
		self.egui_state.process_input(window, event, &mut self.egui_painter);
	}
}
